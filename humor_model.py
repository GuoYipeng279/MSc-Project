...
def step(model, loss_func, data, dataset, device, cur_epoch, mode='train', use_gt_p=1.0):
    ...
    # batch_out, batch_in, meta = data # for reversed HuMoR
...
class HumorModel(nn.Module):
    ...
    def roll_out(self, x_past, init_input_dict, num_steps, use_mean=False, 
        ...
        print('ROLLOUT', x_past.shape, {k:init_input_dict[k].shape if type(init_input_dict[k]) == torch.Tensor else init_input_dict[k] for k in init_input_dict}, 
              num_steps, use_mean, z_seq, return_prior, gender, betas, return_z, canonicalize_input, uncanonicalize_output)
        J = len(SMPL_JOINTS)
        ...
        for t in range(num_steps):
            ...
            print('sample_out', sample_out, x_pred_dict)
            if self.steps_out > 1:
            ...
            #
            # Remember to enable these 3 lines for training
            global_world2local_trans = torch.cat([-cur_world_dict['trans'][:,0:1,:2], torch.zeros((B, 1, 1)).to(x_past)], axis=2)
    ...
    def sample_step(self, past_in, t_in=None, use_mean=False, z=None, return_prior=False, return_z=False, offset=None):
        '''
        ...
        # added for MSc project
        if offset is not None:
            B = offset.size(0)
            past_in = past_in.expand(B,-1)
        ...
        # added for MSc project
        if offset is not None:
            z += offset*torch.sqrt(pv) # control the action by standard diviation. 
